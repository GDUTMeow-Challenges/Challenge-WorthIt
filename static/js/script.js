        ...
        data.items.forEach(item => {
            ...
            <s-card type="outlined" style="padding: 16px;">
                ...
                <div slot="text" style="margin-top: 8px;">
                    ...
                    ${item.properties.附加价值 !== undefined && item.properties.附加价值 !== null ? `<div>附加价值：${item.properties.附加价值} 元</div>` : ''}
                    <div>总价值：${totalValue} 元</div>
                    <div>购买日期：${item.properties.入役日期 || '未填写'}</div>
                    ${item.properties.退役日期 !== undefined && item.properties.退役日期 !== null ? "<div>退役日期：" + item.properties.退役日期 + "</div>" : ""}
...
function triggerBot() {
    fetch("/api/public/trigger");
    showDialog("收到啦！", "收到收到，正在路上(¬◡¬)✧");
}